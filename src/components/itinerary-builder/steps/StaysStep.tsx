...
import { STAY_GROUPS } from '../types/itinerary';
import { cn } from '@/lib/utils';

export function StaysStep() {
  const { state, updateStays, nextStep, prevStep } = useItineraryBuilder();
  const selected = state.itinerary.stays;

  const toggle = (location: string) => {
    if (selected.includes(location)) {
      updateStays(selected.filter((s) => s !== location));
    } else {
      updateStays([...selected, location]);
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h3 className="text-xl font-bold text-gray-900">Night Stay Locations</h3>
